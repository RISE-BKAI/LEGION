...
source_dir = './'
suffix = 'rand123'
model_name = str(sys.argv[1]) # model name as the first argument
threshold_bool = int(sys.argv[2]) # use threshold search or not
loss_funcs = sys.argv[3:] if len(sys.argv) > 2 else [] # loss function name. If unspecified, eval on all loss functions available.
...
# os.path.join(source_dir, 'models')
for j, fname in enumerate(os.listdir(model_dir)):
    if not fname.startswith(".") and not os.path.isdir(os.path.join(model_dir, fname)) and model_name in fname:
        ...
        elif loss_func_name in loss_funcs:
            model_paras.append( (loss_func_name, os.path.join(model_dir, fname)) )
